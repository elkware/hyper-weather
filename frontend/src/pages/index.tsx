...
import {formatWeatherData} from "@/utils/weatherDataUtils";
            ...
            <div className={styles.cardholder}>
                <div className={styles.current}>
                    ...
                    <div className={styles.currentConditions}>
                        <p><Image src={"/" + todayData[0].symbol_code + ".svg"} alt="" width={42}
                                  height={42}/>&nbsp;{todayData[0].symbol_text}</p>
                        <p><span
                            className="material-symbols-outlined">thermometer</span>{todayData[0].air_temperature} °C
                        </p>
                        <p><span
                            className="material-symbols-outlined">humidity_percentage</span> {todayData[0].relative_humidity} %
                        </p>
                        <p><span
                            className="material-symbols-outlined">air</span> {todayData[0].wind_speed} m/s {todayData[0].wind_from_direction}
                        </p>
                        <p><span
                            className="material-symbols-outlined">umbrella</span> {todayData[0].precipitation_amount} mm
                        </p>
                        <p><span className="material-symbols-outlined">cloud</span> {todayData[0].cloud_area_fraction} %
                        </p>
                        <p><span
                            className="material-symbols-outlined">speed</span> {todayData[0].air_pressure_at_sea_level} hPa
                        </p>
                    </div>
                        ...
                        {todayForecast().map((data: {
                            ...
                            <span key={index}>
                                <p>
                                    ...
                                    <span
                                        className="material-symbols-outlined">humidity_percentage</span>{data.relative_humidity}%<br/>
                                    <Image src={"/" + data.symbol_code + ".svg"} alt="" width={18}
                                           height={18}/> {data.air_temperature}°C
                                </p>
                            </span>
                        ))}
                    </div>
                    {todaySummary.report && <div className={styles.todaySummary}><p>Report
                        for {todaySummary.date}:<br/>{todaySummary.report}</p>{todaySummary.report_tts_url &&
                        <PlayAndVisualizeAudio audioUrl={todaySummary.report_tts_url}/>}</div>}
                    ...
                    {Object.entries(otherDaysData).map(([key, value]: [string, Object | any]) => (
                        ...
                        <div key={key}>
                            ...
                            <div className={styles.longtermForecast}>
                                {value[0] &&
                                    <p>dawn<br/><Image src={"/" + value[0].symbol_code + ".svg"} alt="" width={18}
                                                       height={18}/> {value[0].air_temperature + "°C"}</p>}
                                {value[1] &&
                                    <p>morning<br/><Image src={"/" + value[1].symbol_code + ".svg"} alt="" width={18}
                                                          height={18}/> {value[1].air_temperature + "°C"}</p>}
                                {value[2] &&
                                    <p>afternoon<br/><Image src={"/" + value[2].symbol_code + ".svg"} alt="" width={18}
                                                            height={18}/> {value[2].air_temperature + "°C"}</p>}
                                {value[3] &&
                                    <p>dusk<br/><Image src={"/" + value[3].symbol_code + ".svg"} alt="" width={18}
                                                       height={18}/> {value[3].air_temperature + "°C"}</p>}
                            </div>
                        </div>
                    ))}
                </div>
