...
def fetch_data_for_today(location, date):
    day_start = Decimal(
        datetime(date.year, date.month, date.day, 0, 0, 0).timestamp()
    )
    day_end = Decimal(
    ...
    response = hyperweather_data_table.query(
        ...
        & Key("timestamp").between(day_start, day_end)
    )
...
def prepare_prompt(data):
    for item in data:
        item["wind_from_direction"] = wind_direction_to_text(
            item.pop("wind_from_direction")
        )
    ...
    for day_part, chunk in zip(['00:00 to 06:00', '06:00 to 12:00', '12:00 to 18:00', '18:00 to 23:59'], chunks):
        ...
        if min_precipitation_amount == 0 and max_precipitation_amount == 0:
            ...
            prompt += f"Rainfall will be between {min_precipitation_amount} and {max_precipitation_amount} millimeters. "
...
def generate_forcast_report():
    ...
    for city in json.load(open('supported_cities.json', 'r')):
        ...
        item = report_table.get_item(
            Key={'location': location, 'date': date.strftime('%Y-%m-%d')}
        )
